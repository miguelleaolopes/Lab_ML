...
from sklearn.model_selection import cross_val_score, cross_validate, RepeatedStratifiedKFold, KFold, GridSearchCV
...
class alternative_model:
    ...
    def test_model(self, cv_type='KFold'):
        ...
        elif self.cv_type == 'Grid':
            self.scoring = {'accuracy':'accuracy','baccuracy': bca_scorer ,'kappa':kappa_scorer}
            self.param_grid = {'C': [5], 
            'gamma': (np.linspace(0.1, 2, 20)),
            'kernel': ['rbf']} # rbf
            self.grid = GridSearchCV(SVC(), self.param_grid, refit = "baccuracy", verbose = 3, scoring=self.scoring, n_jobs=-1)
            self.fit = self.grid.fit(x_import_small, y_import_small)
        ...
        print("Best Score:",self.fit.best_score_)
        print('Best Hyperparameters: %s' % self.fit.best_params_)


def find_best_svm_gamma(gammas,cv_type='KFold'):
    b_acu = []
    for gamma in gammas:
...
find_best_svm_gamma(np.linspace(1,10,1), cv_type='Grid')